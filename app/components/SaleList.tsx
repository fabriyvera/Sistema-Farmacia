...
const Sales = () => {
    ...
    const fetchSales = async () => {
      try {
        ...
        const res = await fetch(
          "https://690a052a1a446bb9cc2104c7.mockapi.io/Ventas"
        );
  ...
  const getSaleKey = (sale: Venta, index: number) => {
    return (
      sale.id ||
      `sale-${sale.productoId}-${sale.clienteId}-${sale.fecha}-${index}`
    );
  };
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Lista de ventas registradas en la API
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">Bs. {todayTotal}</div>
